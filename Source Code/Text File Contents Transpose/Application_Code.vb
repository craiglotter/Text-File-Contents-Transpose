...
Module Application_Code
    ...
    Sub Main(ByVal sArgs() As String)
        ...
        If Not sArgs.Length = 4 Then
                Console.WriteLine("Text File Contents Transpose")
            Console.WriteLine("-------------------------")
                Console.WriteLine("Usage: executable [affected filename] [input type] [delimiter] [error level]")
            Console.WriteLine("  where:")
            Console.WriteLine("   - [affected filename]: the text file to be parsed")
                Console.WriteLine("   - [input type]: (single, multiple) the input file's current layout")
                Console.WriteLine("   - [delimiter]: string used to either concat or split lines on")
    ...
    Private Sub Log_Handler(ByVal identifier_msg As String)
        Try
            ...
            If dir.Exists = False Then
                dir.Create()
            End If
            Dim filewriter As StreamWriter = New StreamWriter((ApplicationPath() & "\").Replace("\\", "\") & "Activity Logs\" & Format(Now(), "yyyyMMdd") & "_TFCT_Activity_Log.txt", True)
    ...
    Private Sub Error_Handler(ByVal ex As Exception, Optional ByVal identifier_msg As String = "")
        ...
        Catch exc As Exception
            Console.WriteLine("An error occurred in Text File Contents Transpose's error handling routine. The application will try to recover from this serious error.", MsgBoxStyle.Critical, "Critical Error Encountered")
        End Try
    End Sub

    Private Function Execute_Code(ByVal filename As String, ByVal input_type As String, ByVal delimiter As String) As String
        ...
        Try
            ...
            If continue = True Then
                    ...
                    While filereader.Peek > -1
                        ...
                        linetowrite = ""
                        If input_type.ToLower = "single" Then
                            Dim lines() As String = linetocheck.Split(delimiter)
                            For Each str As String In lines
                                If Not (str Is Nothing Or str = "") Then
                                    filewriter.WriteLine(str)
                                End If
                            Next
                        End If
                        If input_type.ToLower = "multiple" Then
                            If filereader.Peek > -1 Then
                                linetowrite = linetocheck & delimiter
                            Else
                                linetowrite = linetocheck
                            End If
